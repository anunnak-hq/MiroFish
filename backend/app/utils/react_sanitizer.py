...
Two-pass design:
  ...
  Pass 2 — Chinese thinking heuristic (conservative): only strips lines that
            start with a known thinking prefix AND contain a tool reference.
"""
...
# ── Pass 2 patterns (Chinese thinking heuristic) ─────────────────────────────

_CHINESE_THINKING_PREFIX = re.compile(
    r'^\s*(让我|我需要|我来|接下来|我尝试|我将)',
)
_TOOL_REFERENCE = re.compile(
    r'quick_search|panorama_search|insight_forge|tool_call|search|分析',
)
...
def sanitize_react_output(text: str) -> str:
    ...
    # Pass 2: Chinese thinking lines (only if ALSO contain tool reference)
    lines = text.split('\n')
    cleaned = []
    for line in lines:
        if _CHINESE_THINKING_PREFIX.match(line) and _TOOL_REFERENCE.search(line):
